        ...
        <div class="relative pt-24 pb-8 px-6">
            <div class="max-w-3xl mx-auto">
                <div class="space-y-4 text-center">
                    <span class="text-[10px] font-bold uppercase tracking-[0.5em] text-gold/60 block">Acquisition Step 02</span>
                    ...
                    <p class="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-4">Select your institutional settlement channel</p>
                </div>
            </div>
        </div>

        <div class="relative max-w-3xl mx-auto px-6 lg:px-8">
            <div class="luxury-card p-8 md:p-12 overflow-hidden" x-data="{ method: '{{ $checkoutData['payment_method'] ?? 'bank_transfer' }}' }">
                <form action="{{ route('checkout.postPayment') }}" method="POST" class="space-y-12">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        ...
                        @if(($settlementSettings['cash_mailing_enabled'] ?? '1') == '1')
                        <!-- Cash Mailing Option -->
                        <label class="relative group cursor-pointer">
                            <input type="radio" name="payment_method" value="cash_mailing" x-model="method" class="hidden peer">
                            <div class="h-full p-8 bg-white/5 border border-white/10 rounded-3xl transition-all duration-500 peer-checked:bg-gold/10 peer-checked:border-gold group-hover:bg-white/[0.08] flex flex-col items-center text-center space-y-4 relative overflow-hidden">
                                <div class="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform duration-500 peer-checked:bg-gold/20">
                                    <svg class="w-8 h-8 text-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                </div>
                                <div>
                                    <h3 class="text-xs font-black uppercase tracking-widest text-white">Cash Mailing</h3>
                                    <p class="text-[9px] font-bold text-gray-500 mt-2 uppercase tracking-tighter">Physical Logistics</p>
                                </div>
                                <div class="absolute top-4 right-4 w-5 h-5 rounded-full border border-white/20 peer-checked:border-gold flex items-center justify-center">
                                    <div class="w-2.5 h-2.5 rounded-full bg-gold opacity-0 peer-checked:opacity-100 transition-opacity"></div>
                                </div>
                    ...
                    <div class="space-y-6 pt-8 border-t border-white/5" x-show="method" x-collapse x-cloak>
                        ...
                        <div x-show="method === 'bank_transfer'" class="space-y-4 animate-fade-in-down">
                            ...
                            <p class="text-[9px] font-bold text-luxury-bronze uppercase tracking-widest ml-1 opacity-60">International wire details will be provided on authorization</p>
                        </div>
                        ...
                        <div x-show="method === 'crypto_transfer'" class="space-y-4 animate-fade-in-down">
                            ...
                            <p class="text-[9px] font-bold text-luxury-bronze uppercase tracking-widest ml-1 opacity-60">Vault address will clear upon acquisition authorization</p>
                        </div>

                        <!-- For Cash Mailing -->
                        <div x-show="method === 'cash_mailing'" class="space-y-4 animate-fade-in-down">
                            <p class="text-[9px] font-bold text-luxury-bronze uppercase tracking-widest ml-1 opacity-60">You will receive mailing logistics and instructions upon authorization. Please ensure you secure tracking for your physical cash transit.</p>
                        </div>
                    </div>

                    <div class="pt-8 flex flex-col md:flex-row gap-4">
                        <a href="{{ route('checkout.details') }}" class="flex-1 text-center bg-white/5 text-gray-400 text-[10px] font-bold py-6 uppercase tracking-[0.3em] hover:bg-white/10 transition-all duration-500 rounded-2xl">
                            Back to Protocols
                        </a>
